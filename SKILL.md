---
name: finscout
description: 下载、提取并分析公司财务报表。支持A股（巨潮资讯网）和港股（港交所披露易）年报下载、PDF转TXT、财务指标提取、Excel数据底稿和HTML分析报告生成。Use when 用户需要从上市公司年报/公告中批量提取财务数据、做同行对比、生成财务分析底稿或报告。
---
...
- 本 Skill 仅用于上市公司财务披露资料下载、数据提取、口径整理和分析报告生成；无关任务不要套用本流程。
- 不得修改用户提供或下载的原始 PDF/TXT 文件；清洗后的 TSV、Excel、HTML、PDF 等结果写入新的输出目录。
- 默认输出目录使用当前工作目录下的 `finance_analysis/data`，除非用户明确指定；不要写入系统目录或个人配置目录。
- 涉及未公开资料、个人信息或内部敏感数据时，在最终交付中提醒用户自行确认脱敏和共享范围。
- 如果官方渠道无法下载、PDF 无法解析、关键数据无法交叉验证，必须明确列出失败原因和已尝试步骤，不得用估算值补齐。
