...
TXT文件动辄上万行，子Agent上下文窗口装不下。用脚本先提取核心报表章节：

```bash
python {SD}/pre_extract.py --input "{txt路径}" --output "{txt路径/.txt/_精简.txt}"
```

脚本自动识别简繁中英章节标题，提取五年财务摘要、BS/PL/CF、分部报告、存货减值附注，将数万行压缩到几千行。子Agent后续只读精简版。
...
=== 任务清单 ===
{逐项列出：公司名、股票代码、年份、精简txt路径、输出TSV路径}

=== 指标清单 ===
{从阶段2d确定的完整指标清单，包括指标名、查找说明、优先级}

=== 港股科目对照表（港股公司必读） ===
港股使用繁体+英文双语报告，科目名与A股完全不同。搜索时必须同时尝试以下关键词：

资产负债表:
  总资产        → 資產總值 / 總資產 / 非流動資產總額+流動資產總額(手动加总)
  总负债        → 負債總值 / 總負債 / 流動負債總額+非流動負債總額(手动加总)
  归母净资产    → 本公司擁有人應佔權益 / 本公司權益持有人應佔權益
  存货          → 存貨 / 物業及其他存貨 / 持作出售物業+發展中物業
  投资性房地产  → 投資物業
  货币资金      → 現金及銀行結餘 / 銀行結餘及現金 / 現金及現金等值
  合同负债      → 合約負債 / 預售樓款
  短期借款      → 借貸(流動部分) / 銀行及其他借貸(一年內到期)
  长期借款      → 借貸(非流動部分) / 銀行及其他借貸(一年後到期)
  应付债券      → 優先票據+中期票據 / 擔保票據及公司債券

利润表:
  营业收入      → 收入 / 營業額 / 營業收入
  营业成本      → 銷售成本 / 直接經營成本
  营业利润      → 經營溢利 / 經營盈利
  归母净利润    → 本公司擁有人應佔溢利 / 本公司權益持有人應佔溢利
  资产减值损失  → 物業減值 / 發展中物業減值
  基本每股收益  → 每股基本盈利

现金流量表:
  经营活动现金流 → 經營活動所得現金淨額 / 經營活動現金流入淨額
  投资活动现金流 → 投資活動所用現金淨額
  筹资活动现金流 → 融資活動所得/(所用)現金淨額

报表名称:
  合并利润表    → 綜合損益表 / 綜合收益表 / Consolidated Statement of Profit or Loss
  合并资产负债表→ 綜合財務狀況表 / Consolidated Statement of Financial Position
  合并现金流量表→ 綜合現金流量表 / Consolidated Statement of Cash Flows

=== 港股BS格式警告 ===
港股常用"净资产法"垂直格式，不出现"总资产""总负债"字样:
  非流動資產總額    XXX
  流動資產總額      XXX
  流動負債總額     (XXX)
  總資產減流動負債  XXX    ← 跳过此行
...
=== 提取要求 ===
1. 读精简txt（不是原始TXT），找到指标清单中每个指标对应的数值
2. 同一指标在不同公司的单位须统一（亿元、%、元/股等），单位不统一的自行换算后再填入
3. 使用 re.DOTALL / 多行搜索：数字可能与科目名不在同一行（PDF转TXT换行导致）
4. 港股报表中搜索指标时，同时搜索科目对照表中的简繁英三语关键词
5. 五年财务摘要页包含大部分核心数据，优先读取
6. 找不到的指标填"-"，备注简述原因
7. 每家公司+期间组合写入一个独立的TSV文件
